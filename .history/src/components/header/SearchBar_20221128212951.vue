<template>
  <div class="relative flex items-center gap-5">
    ...
    >
      ...
      <ul v-if="isNoSearch && isNoResponseData" class="px-3 py-3">
    ...
    async fetchSearch(term) {
      try {
        ...
        if (response.status > 205) {
          thisisNoResponseData = true;
        }
        this.searchResult = response.data.results;
        this.isEmptySearchResult = this.searchResult ? false : true;
      } catch (error) {
        console.log(error);
      }
    },
