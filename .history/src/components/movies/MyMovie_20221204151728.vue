  ...
  <div class="container mx-auto">
      ...
      <div class="ml-24">
        ...
        <div class="flex items-center gap-1">
          ...
          >
            <g transform="translate(0 -1028.4)">
              ...
              <g fill="#e67e22">
                <path
                  d="m22 1038.2c0 0.2-0.104 0.4-0.312 0.6l-4.363 4.2 1.033 6.1c0.008 0 0.012 0.1 0.012 0.2 0 0.2-0.044 0.3-0.132 0.4-0.08 0.1-0.2 0.2-0.361 0.2-0.152 0-0.312 0-0.48-0.1l-5.397-2.9-5.3966 2.9c-0.1763 0.1-0.3366 0.1-0.4808 0.1-0.1683 0-0.2965-0.1-0.3846-0.2-0.0802-0.1-0.1202-0.2-0.1202-0.4 0 0 0.008-0.1 0.024-0.2l1.0337-6.1-4.375-4.2c-0.2003-0.2-0.3005-0.4-0.3005-0.6 0-0.3 0.2244-0.5 0.6731-0.5l6.0336-0.9 2.7043-5.5c0.152-0.3 0.349-0.5 0.589-0.5s0.437 0.2 0.589 0.5l2.704 5.5 6.034 0.9c0.449 0 0.673 0.2 0.673 0.5"
                  fill="#e67e22"
                />
              </g>
              <g fill="#f1c40f">
                <path
                  d="m22 1037.2c0 0.2-0.104 0.4-0.312 0.6l-4.363 4.2 1.033 6.1c0.008 0 0.012 0.1 0.012 0.2 0 0.2-0.044 0.3-0.132 0.4-0.08 0.1-0.2 0.2-0.361 0.2-0.152 0-0.312 0-0.48-0.1l-5.397-2.9-5.3966 2.9c-0.1763 0.1-0.3366 0.1-0.4808 0.1-0.1683 0-0.2965-0.1-0.3846-0.2-0.0802-0.1-0.1202-0.2-0.1202-0.4 0 0 0.008-0.1 0.024-0.2l1.0337-6.1-4.375-4.2c-0.2003-0.2-0.3005-0.4-0.3005-0.6 0-0.3 0.2244-0.5 0.6731-0.5l6.0336-0.9 2.7043-5.5c0.152-0.3 0.349-0.5 0.589-0.5s0.437 0.2 0.589 0.5l2.704 5.5 6.034 0.9c0.449 0 0.673 0.2 0.673 0.5"
                  fill="#f1c40f"
                />
              </g>
            </g>
          ...
          <span class="text-sm text-gray-500"
            >{{ Math.round(CURRENT_MOVIE.vote_average * 10) }}% | {{ CURRENT_MOVIE.release_date }}
            {{ CURRENT_MOVIE_GENRES.join(',') }}</span
          >
        </div>
        <p class="mt-5">
          {{ this.CURRENT_MOVIE.overview }}
        </p>
        <div class="mt-5">
          <span class="mt-5 font-semibold">Featured Cast</span>
          <div class="flex">
            <div v-for="(crew, index) in CURRENT_MOVIE.credits.crew" :key="index">
              <div v-if="index < 2" class="mt-5 mr-5 flex flex-col">
                <span>{{ crew.name }}</span>
        ...
        <div class="mt-5 flex gap-5">
          <a
            ...
            target="blank"
            class="inline-flex cursor-pointer items-center gap-3 rounded bg-yellow-500 px-5 py-3 text-black"
          >
            <svg
              class="w-6"
              xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="border-b border-gray-600 py-4">
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        <CastItem v-for="cast in CURRENT_MOVIE.credits.cast" :key="cast.cast_id" :cast="cast" />
      </div>
    </div>
    <!-- casts-end -->
    ...
    <div class="border-b border-gray-600 py-4">
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        <MovieImagesItem
          v-for="image in getMovieImages"
          :key="image.file_path"
          :image="image"
          @onImageClick="showImageModel"
        />
      </div>
    </div>
    <!-- images-end -->
    ...
    <!-- model -->
    <MediaModel v-model="modelOpen" :mediaURL="mediaURL" :isVideo="isVideo" />
    <!-- model-end -->
  </div>
</template>
...
import CastItem from '../items/CastItem.vue';
import MovieImagesItem from '../items/MovieImagesItem.vue';
...
export default {
  ...
  mounted() {
    const imageSrc = this.actor.profile_path
      ? 'https://image.tmdb.org/t/p/w500/' + this.actor.profile_path
      : 'https://via.placeholder.com/300x450';
    this.SET_IMAGE(imageSrc);
  },
  methods: {
    ...
    playTrailer() {
      if (!this.CURRENT_MOVIE.videos) return;
      return 'https://www.youtube.com/embed/' + this.CURRENT_MOVIE.videos.results[0].key;
    },
  },
  computed: {
    ...mapGetters(['CURRENT_MOVIE', 'CURRENT_MOVIE_GENRES', 'IMAGE']),
    posterPath() {
      return 'https://image.tmdb.org/t/p/original/' + this.CURRENT_MOVIE.poster_path;
    },
    getMovieImages() {
      return this.CURRENT_MOVIE.images.backdrops;
    },
  },
  watch: {
    '$route.params.id': {
      immediate: true,
      handler(newValue) {
        this.FETCH_CURRENT_MOVIE(newValue);
      },
    },
  },
};
