<template>
  <div>
    ...
    <carousel-3d :controls-visible="true" :clickable="false" :listData="comingMovies" :height="500" :key="comingMovies.length">
      <slide v-for="(movie, i) in comingMovies" :index="i" :key="i">
        <router-link :to="`/movie/${movie.id}`">
          <figure>
            <img :src="posterPath(movie)" />
            <figcaption>
              <span class="text-xl tracking-widest">{{ movie.title }}</span>
            </figcaption>
          </figure>
        </router-link>
      </slide>
